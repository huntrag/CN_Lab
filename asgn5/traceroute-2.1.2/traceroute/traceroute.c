...
static int add_gateway(CLIF_option *optn, char *arg)
{
	...
	if (num_gateways >= MAX_GATEWAYS_6)
	{ /*  127 > 8 ... :)   */
...
static CLIF_option option_list[] = {
	...
	{0, "UL", 0, "Use UDPLITE for tracerouting (default dest port is " _TEXT(DEF_UDP_PORT) ")",
...
static void print_probe(probe *pb)
{
	...
	else
	{
		...
		if (np == 0)
		{
			// If you want to know particular hops then uncomment the below code
			printf("\n%2u ", ttl);

			// If you want counters as 1,2,..etc then uncomment the below one
			// printf("\n%2u ", cnt);
			cnt++;
		}
